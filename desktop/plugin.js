...
const REFER_KINDS = new Set(['pdf', 'image', 'markdown', 'html'])
const INTENT_MESSAGE_TYPE = 'hermes-artifact-stage-intent'
const MAX_INTENT_LENGTH = 500

let api = null // bound in register() before any render

function intentScript(token) {
  return '<script>(function(){var t=' + JSON.stringify(token) + ';function send(p){if(typeof p!=="string"||!p.trim())return false;parent.postMessage({type:' + JSON.stringify(INTENT_MESSAGE_TYPE) + ',token:t,prompt:p.slice(0,' + MAX_INTENT_LENGTH + ')},"*");return true}window.hermes={send:send};addEventListener("click",function(e){var el=e.target&&e.target.closest?e.target.closest("[data-hermes-send]"):null;if(el)send(el.getAttribute("data-hermes-send")||"")},true)})()</' + 'script>'
}

function withIntentBridge(documentHtml, token) {
  const script = intentScript(token)
  const bodyEnd = documentHtml.toLowerCase().lastIndexOf('</body>')
  return bodyEnd >= 0
    ? documentHtml.slice(0, bodyEnd) + script + documentHtml.slice(bodyEnd)
    : `${documentHtml}${script}`
}
...
function ArtifactBody({ artifact, seq, zoom, scrollRef, viewportRef, selectionMode, selectionRegion, onRegionSelected, onFramePrompt }) {
  const scaleStyle = {
  ...
  const frameToken = useRef(crypto.randomUUID())
  ...
  useEffect(() => {
    if (artifact.kind !== 'html' || !artifactUrl) {
      setFrameDoc('')
      return undefined
    }
    let live = true
    fetch(artifactUrl).then((response) => response.text()).then((documentHtml) => {
      if (live) setFrameDoc(withIntentBridge(documentHtml, frameToken.current))
    }).catch((error) => {
      if (live) setLoadErr(String((error && error.message) || error))
    })
    return () => { live = false }
  }, [artifact.kind, artifactUrl, seq])
  ...
  useEffect(() => {
    ...
    const receive = async (event) => {
      ...
      if (!message || message.type !== INTENT_MESSAGE_TYPE || message.token !== frameToken.current || typeof message.prompt !== 'string') return
      const prompt = message.prompt.trim().slice(0, MAX_INTENT_LENGTH)
      if (!prompt || Date.now() - lastIntentAt < 1000) return
      lastIntentAt = Date.now()
      let sent = false
      try {
        sent = typeof host.composer?.submit === 'function' && host.composer.submit(null, prompt)
        if (!sent) sent = await onFramePrompt(prompt)
      } catch (_) {
        sent = await onFramePrompt(prompt)
      }
      frameRef.current?.contentWindow?.postMessage({
        type: `${INTENT_MESSAGE_TYPE}-ack`, token: frameToken.current, ok: Boolean(sent),
      }, '*')
    }
    window.addEventListener('message', receive)
    return () => window.removeEventListener('message', receive)
  }, [artifact.id, artifact.title, artifact.kind, onFramePrompt])
...
function StagePane() {
  ...
  const referBar = artifact && REFER_KINDS.has(artifact.kind) && jsxs('form', {
    onSubmit: submitTalk,
