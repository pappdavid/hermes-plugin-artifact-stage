...
def _validate_talk(body: dict) -> dict:
    ...
    if "artifact_title" in body and body["artifact_title"] is not None:
        title = body["artifact_title"]
        if not isinstance(title, str) or not title.strip() or len(title) > 200:
            raise HTTPException(status_code=422, detail="invalid artifact_title")
        entry["artifact_title"] = title.strip()
